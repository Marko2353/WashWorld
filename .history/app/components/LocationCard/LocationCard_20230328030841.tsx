...
export default function LocationCard({ name, status, url }: any) {
  return (
    <div
      className={
        status != "available"
          ? "max-w-sm mx-2 my-4 overflow-hidden shadow-lg bg-slate-200 cursor-not-allowed"
          : "max-w-sm mx-2 my-4 overflow-hidden shadow-lg bg-grey"
      }
    >
      ...
      <div className="flex justify-end">
        {status == "available" ? (
          ...
          <a className="flex items-center justify-center w-1/3 h-full px-4 py-2 mt-8 font-bold text-white transform -skew-x-12 bg-green-500">
            Læs mere
          </a>
        )}
      </div>
